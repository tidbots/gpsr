...
class GpsrAsrLogger:
    def __init__(self):
        ...
        self.max_age_sec = float(rospy.get_param("~max_age_sec", 30.0))  # utterance_end から遡って有効とみなす text の鮮度
        ...
        rospy.loginfo("gpsr_asr_logger ready: text=%s conf=%s end=%s -> %s",
                      self.text_topic, self.conf_topic, self.utt_end_topic, self.out_path)

    def _on_text(self, msg: String):
        self.latest.text = msg.data
        self.latest.t_wall = time.time()
    ...
    def _on_utt_end(self, msg: Bool):
        ...
        age = time.time() - (self.latest.t_wall or 0.0)
        if age > self.max_age_sec or not self.latest.text.strip():
            rospy.logwarn("gpsr_asr_logger: utterance_end but no fresh text (age=%.3f, text='%s')",
                          age, self.latest.text)
            return

        rec = {
            "ts": now_iso(),
            "t_wall": time.time(),
            "text": self.latest.text.strip(),
